...
module.exports.ProcessCallResponse = async (event: HandlerEvent) => {
  ...
  try {
    const MEMBERS = GET_MEMBERS(PARAMS.Client);
    // trigger success condition
    console.log("PROCESS CALL RESPONSE CALLED:", PARAMS)
    const { Called, Digits, Command, CALL_INDEX, CallStatus } = PARAMS;
    ...
    if (parseInt(Digits) === 1) {
      ...
      parseInt(Digits) == 2 // User declined
      || Digits === "TIMEOUT"
    ) {
    ...
    } else if (["failed", "busy", "no-answer"].includes(CallStatus)) // Call failed to reach the user)
    {
      console.log(fname, "wasn't reachable -", CallStatus, "- let's try the next member...");
      // Call next member
      await ReceiveAlert({
        Command,
        CALL_INDEX: parseInt(CALL_INDEX) + 1,
      ...
      console.log(fname, "answered but didn't press 1 or 2 - trying next member...");
      // We don't need to do anything here, it's handled by the <Gather>  timeout
      // Call next member
      // await ReceiveAlert({
      //   Command,
      //   CALL_INDEX: parseInt(CALL_INDEX),
      //   Client: PARAMS.Client,
      //   Alert_Type: PARAMS.Alert_Type
      // }
      // )
    } else if (CallStatus === "in-progress") {
      console.log(fname, "is still on the line - let's wait for them to press 1 or 2...");
    } else {
